...
> **How this project is run:** it is maintained by one person, so the "community leaders"
> below are in practice a single maintainer, and reports are handled on a best-effort
> basis. Reporting routes are GitHub-only — see [Enforcement](#enforcement).
...
community leaders responsible for enforcement **through GitHub**:

* **Publicly** — open an issue in this repository describing the incident, or
* **Privately** — use this repository's **Security** tab → **Report a vulnerability**,
  which is the only private reporting channel this project has. Put "Code of Conduct" in
  the title so it is triaged as a conduct report rather than a security finding.
* **If private reporting is not available to you** — that tab depends on a repository
  setting that may not be enabled. In that case, open an issue saying only that you have a
  Code of Conduct report and need a private channel, with **no details of the incident**,
  and you will be given one.

This project publishes no contact email address; the routes above are the whole list. All
